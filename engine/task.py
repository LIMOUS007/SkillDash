...
def generate_comparison(log_difficulty):
    ...
    comparisons = set()
    graph = defaultdict(set)
    target_edges = max(2, no_of_objects // 2) 
    while len(comparisons) < target_edges:
    ...
    for _ in range(1000):
        ...
        if has_path(graph, x, y) or has_path(graph, y, x):
            if (x, ">", y) not in comparisons and (y, ">", x) not in comparisons:
                break
        else:
            return generate_comparison(log_difficulty)
